...
def main():
    solver = Solver(args, source=args.source, target=args.target, learning_rate=args.lr, batch_size=args.batch_size,
                    optimizer=args.optimizer, all_use=args.all_use,
                    entropy=args.ent, checkpoint_dir=args.checkpoint_dir, save_epoch=args.save_epoch)
    record_train = 'record/%s_%s_alluse_%s_entropy_%s.txt' % (
        args.source, args.target, str(args.all_use), str(args.ent))
    record_test = 'record/%s_%s_alluse_%s_entropy_%s_test.txt' % (
        args.source, args.target, str(args.all_use), str(args.ent))
